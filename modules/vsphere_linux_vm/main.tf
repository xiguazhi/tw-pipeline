...
resource "vsphere_virtual_machine" "Linux" {
  ...
  name       = "${var.dc_abreviation}${var.environment}-${var.vmname}0${count.index + 1}"
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name    = "%{if var.vmnameliteral != ""}${var.vmnameliteral}%{else}${var.dc_abreviation}${var.environment}-${var.vmname}${count.index + 1}${var.vmnamesuffix}%{endif}"
        domain       = var.vmdomain
...
resource "vsphere_virtual_machine" "Windows" {
  ...
  clone {
    ...
    customize {
      windows_options {
        computer_name         = "%{if var.vmnameliteral != ""}${var.vmnameliteral}%{else}${var.dc_abreviation}${var.environment}-${var.vmname}${count.index + 1}${var.vmnamesuffix}%{endif}"
